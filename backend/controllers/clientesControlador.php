    ...
    case 'insertar':
        ...
        try {
            ...
            if ($esFormData) {
                ...
                // Si el cliente ya existe, rechazar con error claro
                if ($clienteExistente) {
                    echo json_encode([
                        "resultado" => "error",
                        "mensaje" => "Ya existe un cliente registrado con el documento $documentoCliente. Si desea modificar sus datos, use la opción Editar."
                    ]);
                    exit;
                }

                // A partir de aquí, el documento NO existe, así que continuamos creando el cliente nuevo
                ...
                if ($tieneFiador) {
                    ...
                    if ($fiadorExistente) {
                        ...
                        // Actualizar datos del fiador existente (conservando valores si vienen vacíos)
                        $paramsFiador = [
                            'documento' => $documentoFiador,
                            'nombres' => trim($_POST['nombres_fiador'] ?? '') ?: $fiadorExistente['nombres'],
                            'apellidos' => trim($_POST['apellidos_fiador'] ?? '') ?: $fiadorExistente['apellidos'],
                            'direccion' => trim($_POST['direccion_fiador'] ?? '') ?: $fiadorExistente['direccion'],
                            'telefono' => trim($_POST['telefono_fiador'] ?? '') ?: $fiadorExistente['telefono'],
                            'telefono2' => trim($_POST['telefono2_fiador'] ?? '') ?: $fiadorExistente['telefono2'],
                            'activo' => 1,
