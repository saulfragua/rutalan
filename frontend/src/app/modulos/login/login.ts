...
declare var recaptchaReady: boolean; // ← declara la variable global
...
export class Login implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    if (typeof recaptchaReady !== 'undefined' && recaptchaReady && typeof grecaptcha !== 'undefined') {
      // El script ya estaba cargado (venimos de navegación SPA)
      this.doRender();
    } else {
      // Primera carga: esperamos el evento
      this.recaptchaListener = () => this.doRender();
      window.addEventListener('recaptchaready', this.recaptchaListener);
    }
  }

  ngOnDestroy(): void {
    if (this.recaptchaListener) {
      window.removeEventListener('recaptchaready', this.recaptchaListener);
    }
  }

  private doRender(): void {
    const contenedor = document.getElementById('recaptchaContainer');
    if (contenedor && contenedor.childElementCount === 0) {
      this.recaptchaWidgetId = grecaptcha.render('recaptchaContainer', {
        sitekey: '6Le1qg0tAAAAABKHtgcnBB6Ywj6ikiunpGCWu2CG'
      });
    }
  }
  ...
  iniciarSesion() {
    ...
    const token = this.recaptchaWidgetId !== null
